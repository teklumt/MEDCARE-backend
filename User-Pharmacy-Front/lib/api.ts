    ...
    const detail =
      typeof data.details === 'string'
        ...
        : data.details != null && typeof data.details === 'object'
          ...
          : undefined;
    throw new ApiRequestError(data.error || data.message || 'Request failed', response.status, detail);
  }
...
export const addUserAddress = async (address: UserAddress): Promise<UserAddress[]> => {
  const res = await apiPost<UserAddress[]>('/users/me/addresses', address);
  return res.data ?? [];
};

export const updateUserAddress = async (id: string, address: Partial<UserAddress>): Promise<UserAddress[]> => {
...
export const changeMyPassword = async (body: ChangeMyPasswordBody): Promise<void> => {
...
export const getPharmacyById = async (id: string): Promise<PharmacyListItem> => {
...
export const updateMyPharmacy = async (payload: UpdateMyPharmacyPayload): Promise<MyPharmacyProfile> => {
  const res = await apiPut<MyPharmacyProfile>('/pharmacy/me', payload);
  if (!res.data) throw new Error(res.error || res.message || 'Failed to update pharmacy');
  return res.data;
};
...
export const getPharmacyInventory = async (
  ...
  const res = await apiGet<{ items?: PharmacyMedicationItem[] } | PharmacyMedicationItem[]>(
    `/pharmacies/${pharmacyId}/inventory`,
    params
  );
  const data = res.data;
  if (Array.isArray(data)) return data;
  if (data && Array.isArray((data as { items?: PharmacyMedicationItem[] }).items)) {
...
export const submitPharmacyReview = async (
  ...
  const res = await apiPost<PharmacyReviewItem>(`/pharmacies/${pharmacyId}/reviews`, body);
  if (!res.data) throw new ApiRequestError('Review submission returned no data', 500);
  return res.data;
};
...
export const submitComplaint = async (body: SubmitComplaintBody): Promise<ComplaintRecord> => {
  const res = await apiPost<ComplaintRecord>('/complaints', body);
  if (!res.data) throw new Error(res.error || res.message || 'Failed to submit complaint');
  return res.data;
};
...
export const getMyDeliveryAgents = async (): Promise<PharmacyDeliveryAgent[]> => {
  const res = await apiGet<PharmacyDeliveryAgent[]>('/pharmacy/me/delivery-agents');
  return res.data ?? [];
};
...
export const setMyDeliveryOnlineStatus = async (isOnline: boolean): Promise<boolean> => {
  const res = await apiPatch<{ isOnline: boolean }>('/delivery/me/status', { isOnline });
  if (!res.success) {
    throw new Error(res.error || res.message || 'Failed to update online status');
  }
  return Boolean(res.data?.isOnline);
};

/** Dispatched orders assigned to the logged-in delivery agent. */
export const getMyDeliveryOrders = async (): Promise<DeliveryAssignedOrder[]> => {
  const res = await apiGet<DeliveryAssignedOrder[]>('/delivery/me/orders');
  return res.data ?? [];
};

export const startDeliveryTrip = async (orderMongoId: string): Promise<DeliveryAssignedOrder> => {
  const res = await apiPatch<DeliveryAssignedOrder>(`/delivery/orders/${orderMongoId}/start-trip`, {});
  if (!res.data) throw new Error('No order returned');
  return res.data;
};

export const confirmDriverHandoff = async (orderMongoId: string): Promise<DeliveryAssignedOrder> => {
  const res = await apiPatch<DeliveryAssignedOrder>(`/delivery/orders/${orderMongoId}/confirm-handoff`, {});
  if (!res.data) throw new Error('No order returned');
  return res.data;
};
...
export const getMyDeliveryHistory = async (params?: {
  ...
  if (!res.data) throw new Error(res.error || res.message || 'Failed to load delivery history');
  return res.data;
};

export const getMyDeliveryEarnings = async (): Promise<DeliveryEarningsResponse> => {
...
export const confirmOrderReceipt = async (orderMongoId: string): Promise<OrderDetail> => {
  const res = await apiPatch<OrderDetail>(`/orders/${orderMongoId}/confirm-receipt`, {});
  if (!res.data) throw new Error('No order returned');
  return res.data;
};
...
export type OrderDetail = {
  ...
  deliveryAddress?: DeliveryAddressPayload;
  deliveryInstructions?: string;
  items: Array<{ medicationName: string; quantity: number; unitPrice?: number; subtotal?: number }>;
  createdAt: string;
  pharmacyId?: unknown;
...
export const getOrderTracking = async (orderId: string): Promise<OrderTrackingData> => {
  const res = await apiGet<OrderTrackingData>(`/orders/${orderId}/tracking`);
  if (!res.data) throw new Error('Tracking not available');
  return res.data;
};

export const updateDriverLocation = async (lat: number, lng: number): Promise<void> => {
...
export const scanPrescription = async (file: File): Promise<ScanPrescriptionData> => {
  const formData = new FormData();
  formData.append('file', file);
  const res = await apiPost<ScanPrescriptionData>('/prescriptions/scan', formData);
  if (!res.data) throw new Error(res.error || res.message || 'Prescription scan failed');
  return res.data;
};
...
): Promise<{ order: OrderDetail & { _id?: string }; payment: unknown | null }> => {
  const res = await apiPost<{ order: OrderDetail & { _id?: string }; payment: unknown | null }>('/orders', {
    ...payload,
    deferRxReview: true
  });
...
export const completeOrderPayment = async (
  ...
  const res = await apiPost<{ order: OrderDetail; payment?: unknown; checkoutUrl?: string }>(
...
export const getPrescriptionUpload = async (id: string): Promise<PrescriptionUploadDetail> => {
...
export const rejectPrescriptionUpload = async (id: string, note?: string): Promise<void> => {
...
export const sendMedcareAiChat = async (payload: {
  ...
  if (payload.conversationId != null && String(payload.conversationId).trim() !== '') {
    body.conversationId = String(payload.conversationId).trim();
  }
  const res = await apiPost<MedcareAiChatResponseData>('/medcare-ai/chat', body);
  if (!res.data) throw new Error(res.error || res.message || 'MedCare AI request failed');
  return res.data;
};
...
export const createConversation = async (payload: CreateConversationPayload): Promise<ConversationListItem> => {
...
export const sendConversationMessage = async (
  ...
  const res = await apiPost<ConversationMessage>(`/conversations/${conversationId}/messages`, { content });
