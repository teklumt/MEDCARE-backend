...
export default function AddStockPanel({ isOpen, onClose, onAdd }: AddStockPanelProps) {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setFormData({
        ...
        availability: 'Available Now', deliveryEligible: true, maxOrderQty: '',
  ...
  const handleSubmit = () => {
    if (validateForm()) {
      const newItem = {
        ...
        status: formData.availability === 'Available Now' ? 'In Stock' : 'Hidden',
      };
    ...
    <div className="fixed inset-0 z-50 flex justify-end bg-gray-900/50 backdrop-blur-sm transition-opacity">
      <div className="w-full max-w-2xl bg-white h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
        ...
        <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between bg-white shrink-0">
          <h2 className="text-xl font-serif font-bold text-brand-950">Add New Stock</h2>
        ...
        {step === 'success' ? (
          <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
            ...
            <h3 className="text-2xl font-bold text-brand-950 mb-2">Stock Added Successfully</h3>
            <p className="text-gray-600 mb-8">
              <span className="font-bold text-brand-900">{formData.nameEn}</span> has been added to your inventory. It is now visible to patients.
            </p>
              ...
              <section className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm space-y-5">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  ...
                  <div className="md:col-span-2">
                    <label className="block text-sm font-bold text-gray-700 mb-1">Description <span className="text-gray-500 font-normal ml-1">(Optional details about usage, purpose, etc.)</span></label>
                    <textarea 
                      rows={3}
                    ...
                    <div className="flex flex-col gap-2">
                      <label className="flex items-center gap-2 cursor-pointer">
                        <input 
                          ...
                          className="w-4 h-4 text-brand-600 focus:ring-brand-500"
                        />
                        <span className="text-sm">Available Now <span className="text-gray-500 text-xs">(Visible to patients)</span></span>
                      </label>
                      <label className="flex items-center gap-2 cursor-pointer">
                        <input 
                          type="radio" 
                          ...
                          checked={formData.availability === 'Mark as Unavailable'}
                          onChange={(e) => setFormData({...formData, availability: e.target.value})}
                          className="w-4 h-4 text-brand-600 focus:ring-brand-500"
                        />
                        <span className="text-sm">Mark as Unavailable <span className="text-gray-500 text-xs">(Hidden from search)</span></span>
                      </label>
                    </div>
                  </div>
