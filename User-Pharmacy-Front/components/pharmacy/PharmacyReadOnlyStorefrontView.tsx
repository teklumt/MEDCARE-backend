...
import type { PharmacyListItem, PharmacyMedicationItem, PharmacyReviewItem } from '@/lib/api';
...
export type PharmacyReadOnlyStorefrontTone = 'admin' | 'pharmacy_portal_preview';

export function PharmacyReadOnlyStorefrontView({
  pharmacy,
  medications,
  reviewRows,
  reviewsTotal,
  ...
  reviewRows: PharmacyReviewItem[];
  reviewsTotal: number;
  reviewsError: string | null;
  tone: PharmacyReadOnlyStorefrontTone;
}) {
  ...
  const showCallNow = tone === 'pharmacy_portal_preview';
  ...
  const asideNote =
    tone === 'admin' ? (
      ...
      <p className="rounded-2xl border border-dashed border-brand-200 bg-brand-50/60 px-4 py-3 text-center text-sm text-brand-950">
        Read-only preview. Patients sign in on MedCare to place orders or leave reviews.
      </p>
    );

  const medCaption =
    tone === 'admin' ? 'Preview only — not sold from Admin' : 'Preview only — patients add items from their MedCare account';
    ...
    <>
      ...
      >
        ...
        {medications.length === 0 ? (
          <p className="py-8 text-center text-gray-500">No medications listed for this pharmacy yet.</p>
        ) : (
                ...
                >
                  ...
                  <div className="mt-auto space-y-3 border-t border-gray-50 pt-4">
                    <div className="mb-2 flex items-center justify-between">
                      <span className="text-lg font-bold text-gray-900">{Number(med.price).toFixed(2)} ETB</span>
                      {inStock ? (
                    ...
                    <p className="text-center text-xs font-medium text-gray-500">{medCaption}</p>
                  </div>
                </div>
              );
            })}
