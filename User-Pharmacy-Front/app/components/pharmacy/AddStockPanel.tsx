...
import { uploadPharmacyMedicationImageApi } from "@/lib/api";
...
export default function AddStockPanel({
  ...
  const [formData, setFormData] = useState({
    ...
    prescriptionMode:
      "Require prescription image upload before order is confirmed",
    ...
    if (isOpen) {
      ...
      if (editItem) {
        setFormData({
          ...
          dateReceived:
            editItem.dateReceived || new Date().toISOString().split("T")[0],
  ...
  const handleImageSelect = async (file: File) => {
    ...
    try {
      ...
      setImageError(null);
      const url = await uploadPharmacyMedicationImageApi(file);
      setImageUrl(url);
    } catch (err) {
      setImageError((err as Error).message || "Failed to upload image");
    } finally {
      setImageUploading(false);
    }
  };
    ...
    if (formData.expirationDate) {
      ...
      if (diffDays > 0 && diffDays <= 30) {
        setWarnings((prev) => ({
          ...prev,
          expirationDate:
            "⚠️ This item will expire soon. It will appear with an expiration warning to you but will still be listed as available to patients until it expires.",
        }));
      } else {
        setWarnings((prev) => {
          const newWarn = { ...prev };
  ...
  const validateForm = () => {
    ...
    if (!formData.nameEn.trim())
      newErrors.nameEn = "Medication Name in English is required.";
    if (!formData.nameAm.trim())
      newErrors.nameAm = "Medication Name in Amharic is required.";
    if (!formData.dosageForm) newErrors.dosageForm = "Dosage Form is required.";
    if (!formData.category) newErrors.category = "Category is required.";
    if (formData.requiresPrescription === null)
      newErrors.requiresPrescription =
        "Please select if a prescription is required.";
        ...
        {step === "success" ? (
          <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
            ...
            <p className="text-gray-600 mb-8">
              ...
              has been {editItem ? "updated in" : "added to"} your inventory. It
              is now visible to patients.
            </p>
          ...
          <>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-8 bg-accent-50/30">
              ...
              <section className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm space-y-5">
                <h3 className="font-bold text-brand-950 border-b border-gray-100 pb-2">
                    ...
                    <div className="absolute z-10 w-full mt-1 bg-white border border-gray-200 rounded-xl shadow-lg max-h-48 overflow-y-auto">
                      ...
                      {MASTER_DB.filter((m) =>
                        ...
                        <div className="px-4 py-3 text-sm text-gray-500">
                          No matches found. You will create a new entry.
                        </div>
                      )}
                    </div>
                  )}
                ...
                {isNewMedication && searchQuery.length > 0 && (
                  <div className="bg-blue-50 border border-blue-200 text-blue-800 p-3 rounded-xl text-sm flex gap-2 items-start">
                    ...
                    <p>
                      This medication is not in our database yet — you will
                      create a new entry.
                    </p>
                  </div>
                )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  ...
                  <div>
                    <label className="block text-sm font-bold text-gray-700 mb-1">
                      Name in Amharic <span className="text-red-500">*</span>
                    </label>
                    <input
                      type="text"
                      value={formData.nameAm}
                      onChange={(e) =>
                        setFormData({ ...formData, nameAm: e.target.value })
                      }
                  ...
                  <div className="md:col-span-2">
                    <label className="block text-sm font-bold text-gray-700 mb-1">
                      Description of the medication/drug{" "}
                      <span className="text-gray-500 font-normal ml-1">
                        (Details for what purpose it is used for)
                      </span>
                    </label>
                    <textarea
                      rows={3}
                  ...
                  <div>
                    <label className="block text-sm font-bold text-gray-700 mb-1">
                      ...
                      <span
                        ...
                        title="Batch numbers help identify recalled medications."
                      >
                        ⓘ
                      </span>
                    </label>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setFormData({ ...formData, storageReq: e.target.value })
                      }
                      className="w-full px-4 py-2.5 bg-white border border-gray-300 rounded-xl outline-none focus:ring-2 focus:ring-brand-500"
                    >
                      {STORAGE_REQS.map((s) => (
                    ...
                    <div>
                      ...
                      {formData.isControlled && (
                        <p className="text-xs text-amber-700 mt-1 bg-amber-50 p-2 rounded-lg border border-amber-200">
                          By marking this as a controlled substance, you confirm
                          this medication is stored and dispensed in compliance
                          with Ethiopian pharmaceutical regulations.
                        </p>
                      )}
                    </div>
                  </label>
              ...
              <section className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm space-y-5">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    >
                      <option value="In Stock">
                        In Stock (Available to patients)
                      </option>
                      <option value="Low Stock">
                        Low Stock (Available but running out)
                      </option>
                      <option value="Out of Stock">
                        Out of Stock (Hidden from search/Cannot order)
                      </option>
                      <option value="Expiring Soon">
                        Expiring Soon (Visible with warning)
                      </option>
                    </select>
                  </div>
                  ...
                  <div className="flex items-center justify-between py-3 border-t border-gray-100">
                    <div>
                      ...
                      <span className="text-xs text-gray-500">
                        Can this item be delivered to patients?
                      </span>
                    </div>
                  ...
                  <div>
                    <label className="block text-sm font-bold text-gray-700 mb-1">
                      Max Order Quantity per Patient
                    </label>
                    <input
                      type="number"
              ...
              {formData.requiresPrescription && (
                <section className="bg-brand-50 p-6 rounded-2xl border border-brand-200 shadow-sm space-y-5">
                  <h3 className="font-bold text-brand-950 border-b border-brand-200 pb-2">
                    5. Prescription Handling Settings
                  </h3>
                      ...
                      <div className="flex flex-col gap-3">
                        <label className="flex items-start gap-2 cursor-pointer">
                          <input
                            ...
                            checked={
                              formData.prescriptionMode ===
                              "Require prescription image upload before order is confirmed"
                            }
                            onChange={(e) =>
                              setFormData({
                                ...formData,
                                prescriptionMode: e.target.value,
                              })
                            }
                            className="mt-1 w-4 h-4 text-brand-600 focus:ring-brand-500"
                          />
                          <span className="text-sm">
                            Require prescription image upload before order is
                            confirmed{" "}
                        ...
                        <label className="flex items-start gap-2 cursor-pointer">
                          <input
                            ...
                            checked={
                              formData.prescriptionMode ===
                              "Accept order but request prescription at delivery or pickup"
                            }
                            onChange={(e) =>
                              setFormData({
                                ...formData,
                          ...
                          />
                          <span className="text-sm">
                            Accept order but request prescription at delivery or
                            pickup
                          </span>
                        </label>
                        <label className="flex items-start gap-2 cursor-pointer">
                          <input
                            ...
                            checked={
                              formData.prescriptionMode ===
                              "Pharmacist reviews prescription image manually before accepting order"
                            }
                            onChange={(e) =>
                              setFormData({
                                ...formData,
                          ...
                          />
                          <span className="text-sm">
                            Pharmacist reviews prescription image manually
                      ...
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          onChange={(e) =>
                            setFormData({
                              ...formData,
                              prescriptionValidDays: e.target.value,
                            })
                          }
              ...
              <section className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm space-y-5">
                ...
                {imageUrl ? (
                  <div className="space-y-3">
                    <div className="relative rounded-2xl overflow-hidden border border-gray-200 bg-gray-50 aspect-video max-h-48">
                  ...
                  <button
                    ...
                    onClick={() => imageInputRef.current?.click()}
                    disabled={imageUploading}
                    className="w-full border-2 border-dashed border-gray-300 rounded-2xl p-8 text-center hover:bg-gray-50 transition-colors cursor-pointer disabled:opacity-70"
                  >
                    {imageUploading ? (
                      <Loader2 className="w-10 h-10 text-brand-600 mx-auto mb-3 animate-spin" />
                    ) : (
                      <div className="w-12 h-12 bg-brand-50 text-brand-600 rounded-full flex items-center justify-center mx-auto mb-3">
                        <Camera className="w-6 h-6" />
                      </div>
                    )}
                    <p className="font-bold text-brand-950 mb-1">
                    ...
                    <p className="text-xs text-gray-400 max-w-sm mx-auto">
                      A clear photo helps patients verify they received the
                      correct medication and reduces dispensing errors.
                    </p>
                  </button>
                )}
                {imageError && (
            ...
            <div className="p-4 border-t border-gray-200 bg-white shrink-0 flex justify-end gap-3">
              ...
              <button
                ...
                disabled={imageUploading}
                className="px-8 py-2.5 bg-brand-900 text-white font-bold rounded-xl hover:bg-brand-800 transition-colors shadow-sm disabled:opacity-70"
              >
                {imageUploading
                  ? "Uploading image..."
                  : editItem
