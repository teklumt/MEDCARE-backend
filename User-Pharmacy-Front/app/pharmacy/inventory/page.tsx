...
import { Search, Plus, Upload, AlertTriangle, Clock, Edit, Trash2, Globe, ChevronDown } from 'lucide-react';
...
const mapMedicationToItem = (med: any): InventoryItem => {
  ...
  const expiringSoon = expiryDate ? expiryDate <= new Date(Date.now() + 30 * 24 * 60 * 60 * 1000) : false;
  let status = 'In Stock';

  if (med.stockStatus === 'out_of_stock') status = 'Out of Stock';
  if (med.stockStatus === 'low_stock') status = 'Low Stock';
  if (expiringSoon && med.stockStatus !== 'out_of_stock') status = 'Expiring Soon';
...
export default function InventoryPage() {
  ...
  const handleAddStock = async (newItem: any) => {
    try {
      ...
      if (editItem) {
        const response = await apiPatch<any>(`/pharmacy/me/inventory/${editItem.id}`, payload);
        const updatedItem = mapMedicationToItem(response.data);
        setInventoryItems((prev) => prev.map((item) => (item.id === editItem.id ? updatedItem : item)));
        setEditItem(null);
      } else {
        const response = await apiPost<any>('/pharmacy/me/inventory', payload);
        if (response.data) {
          setInventoryItems((prev) => [mapMedicationToItem(response.data), ...prev]);
        }
      }
    } catch (error) {
      console.error(error);
  ...
  const handleUpdateItemField = (id: string, field: 'quantity' | 'price', value: number) => {
    setInventoryItems((prev) => prev.map((item) => (item.id === id ? { ...item, [field]: value } : item)));
  };

  const handleCommitUpdate = async (id: string, payload: Record<string, any>) => {
    try {
  ...
  const handleBulkUpload = async (file: File) => {
    ...
    await apiPost('/pharmacy/me/inventory/bulk-upload', formData);
    const response = await apiGet<any[]>('/pharmacy/me/inventory');
                ...
                >
                  <td className="p-4">
                    <p className="font-bold text-brand-950 text-sm">
                      {language === 'am' && item.amharicName ? item.amharicName : item.name}
                    </p>
                    <p className="text-xs text-gray-500">
                      {language === 'en' && item.amharicName ? item.amharicName : item.name} • {t.batch}:{' '}
                      {item.batch || 'N/A'}
                    </p>
                  </td>
                  ...
                  <td className="p-4">
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        onBlur={(e) => handleCommitUpdate(item.id, { stockQuantity: Number(e.target.value) })}
                  ...
                  <td className="p-4">
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        onBlur={(e) => handleCommitUpdate(item.id, { price: Number(e.target.value) })}
